...
function bundle (parser, options) {
  ...
  if (options.bundle && options.bundle.shallowReferences) {
    inventory = addExternalRefSources(parser, inventory)
...
// add x-refSource to referenced values, indicating what URL they were referenced from
function addExternalRefSources(parser, inventory) {
  return inventory.map((entry) => {
    if (entry.indirections === 0 && entry.external) { // only add refs on schemas referenced in the root
      entry.value['x-refSource'] = entry.file + entry.hash
    }
    return entry
  })
}
...
function addShallowReferences(parser, inventory) {
  inventory = inventory.reduce((inv, entry) => {
    if (entry.external) {
      ...
      const samePathReference = entry.hash === entry.pathFromRoot
      const alreadyPresent = get(parser.schema, entry.hash.substring(1).split('/'))
      if (samePathReference) {
        // silent skip, no need for additional references
      } else if (alreadyPresent) {
        console.log(`Skipping adding a shallow reference for ${entry.$ref.$ref} as this path already exists in the root schema.`)
      } else if (!samePathReference && entry.depth > 2 && (entry.file !== prev.file || (entry.file === prev.file && entry.hash !== prev.hash && entry.hash.indexOf(prev.hash + '/') !== 0))) {
...
function get(obj, pathParts) {
  if (pathParts.length === 1) {
    ...
    const next = obj[pathParts[0]]
    if (next) {
      return get(next, pathParts.slice(1))
    } else {
      return null
    }
  }
